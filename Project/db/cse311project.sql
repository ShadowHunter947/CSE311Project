...
--
-- Host: 127.0.0.1
-- Generation Time: Jan 04, 2022 at 02:59 AM
...
-- Procedures
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `productorder_log` (IN `Id` INT)  SELECT price FROM productorder where UserId=Id$$
...
--
DELIMITER $$
CREATE TRIGGER `delete_log` AFTER DELETE ON `users` FOR EACH ROW INSERT INTO log VALUES (null, old.UserId, "DELETED")
$$
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `update_log` AFTER UPDATE ON `users` FOR EACH ROW INSERT INTO log VALUES (null, new.UserId, "UPDATED")
$$
DELIMITER ;
